...
window.VisitCountProtect = async function ()
{
    //로그 카운트 추가
    ++LogCout;
    ...
    AA2.put(
        AjaxAssist.TokenRelayType.None
        , {
            url: "/api/Site/VisitCountProtect",
            success: async function (data, textStatus, response)
            {
                console.log(LogCout + " 카운트 추가 성공");
                await VisitCountGet();
...
window.VisitCountProtect2 = async function ()
{
    ...
    console.log(LogCout + " 카운트 추가2 시도");
    ...
    AA2.put(
        ...
        , {
            ...
            success: async function (data, textStatus, response)
            {
                console.log(LogCout + " 카운트 추가2 성공");
                await VisitCountGet();
            },
            error: function (response, textStatus, errorThrown)
            {
...
window.VisitCountProtectLoop = async function ()
{
    //로그 카운트 추가
    ++LogCout;
